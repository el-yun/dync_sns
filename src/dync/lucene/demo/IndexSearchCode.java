			...
			for (int j = 0; j < splitKeyword.length; j++) {
				...
				if(j==FINDCONDITIONAND){
					for(int k = 0; k < splitKeyword.length ; k++){
						Term term = new Term(IndexConstants.KEY_CODECONTENTS, splitKeyword[k]);
				...
				if (count == 0)
					...
					for (int i = 0; i < count; i++) {
						...
						arrResult
								.add(selectDoc.get(IndexConstants.KEY_CODEID));
					}
				}
			}
